...
2. **解析用户输入**：提取场景、姿势、服装、表情、氛围中用户明确提到的要素。未提及的要素从 `preferences` 的偏好设定中补全。
...
4. **按五层权重金字塔构建提示词**：
   ...
   - **层5 服装与细节**：从用户指定或 `preferences` 服装池选取；袜子统一使用 perfect smooth flawless legs under stockings, delicate lace naturally adhering without visible marks——正面描述腿部质感，避免压痕词汇触发过滤
...
| v1.3 | 2026-03-13 | Dona / Claude | 第2步、第4步层5中 core-anchors 章节1 残留引用改为 preferences | 消除 v1.2 遗漏的两处残留引用 |
